...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:container_block`, {
        onPlace({ block, dimension }) {
            ...
            function spawnContainerEntity(block) {
...
system.afterEvents.scriptEventReceive.subscribe(({ sourceEntity, message }) => {
    ...
    if (sourceEntity.typeId !== "maca_vf:container_util")
        return;
    // 
...
world.afterEvents.playerInteractWithEntity.subscribe(event => {
    ...
    if (target.typeId == "maca_vf:container_util") {
        ...
        // update block state
        block.setPermutation(block.permutation.withState(`${NAMESPACE}:open`, true));
...
function onContainerExit(player, entityId, reason) {
    ...
    // update block state
    block.setPermutation(block.permutation.withState(`${NAMESPACE}:open`, false));
}
