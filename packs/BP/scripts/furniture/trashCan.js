...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:trash_can`, {
        onPlayerInteract({ block, player, dimension }) {
            const isTrashCanOpen = block.permutation.getState(`${NAMESPACE}:trash_can_open`);
            // check if the state
            if (isTrashCanOpen) {
                // playsound
                ...
                // update block state
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:trash_can_open`, false));
            }
            else {
                // playsound
                block.dimension.playSound("open_trapdoor.copper", block.center());
                // update block state
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:trash_can_open`, true));
            }
        }
    });
});
