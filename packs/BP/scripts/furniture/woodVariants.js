        ...
        onPlayerInteract({ block, player, dimension }) {
            ...
            if (selectedItem && selectedItem.typeId == "minecraft:wooden_axe") {
                ...
                if (Multiblock.isMultiblock(block)) {
                    new Multiblock(block, -1).setMultiblockState(`${NAMESPACE}:wood_variant`, woodVariant == 11 ? 0 : woodVariant + 1);
                }
                else {
                    block.setPermutation(block.permutation.withState(`${NAMESPACE}:wood_variant`, woodVariant == 11 ? 0 : woodVariant + 1));
                }
                // playsound
