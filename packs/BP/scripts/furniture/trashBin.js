    ...
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:trash_bin`, {
        onPlayerInteract({ block, player, dimension }) {
            ...
            if (selectedItem && dyeList.has(selectedItem.typeId))
                return;
            // get block state
            const isTrashBinOpen = block.permutation.getState(`${NAMESPACE}:trash_bin_open`);
            // check if the state
            if (isTrashBinOpen) {
                // playsound
                ...
                // update block state
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:trash_bin_open`, false));
            }
            else {
                // playsound
                block.dimension.playSound("open_trapdoor.copper", block.center());
                // update block state
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:trash_bin_open`, true));
            }
        }
    });
});
