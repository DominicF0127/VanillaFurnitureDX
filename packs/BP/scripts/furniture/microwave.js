...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:microwave`, {
        onPlace({ block }) {
            world.setDynamicProperty(blockGeneratedDynamicPropertyId(block), JSON.stringify({ status: 0, input: "", output: "" }));
        },
        onPlayerInteract({ block, player }) {
            const dynamicPropertyId = blockGeneratedDynamicPropertyId(block);
            const cooking = JSON.parse(world.getDynamicProperty(dynamicPropertyId));
            if (cooking.status !== 0)
                return;
            const isMicrowaveOpen = block.permutation.getState(`${NAMESPACE}:microwave_open`);
            ...
            if (cooking.status == 0 && (!isMicrowaveOpen || !selectedItem || !rawFoodList.has(selectedItem.typeId))) {
                ...
                // update block state
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:microwave_open`, isMicrowaveOpen ? false : true));
                return;
            }
            ...
            // update block state
            block.setPermutation(block.permutation.withState(`${NAMESPACE}:microwave_open`, false));
        ...
        onTick({ block, dimension }) {
            ...
            if (cooking.status !== 10) {
                ...
                // update block state
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:microwave_open`, true));
                // update oven data then save
                cooking.status = 0;
                cooking.input = "";
                cooking.output = "";
