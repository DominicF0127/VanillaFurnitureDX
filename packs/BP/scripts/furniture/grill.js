...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:grill`, {
        ...
        onPlayerBreak({ block, dimension }) {
            // get data
            const cooking = JSON.parse(world.getDynamicProperty(blockGeneratedDynamicPropertyId(block)));
            // check status
            if (cooking.status == 0)
                return;
            // spawn input item
        ...
        onPlayerInteract({ block, player, dimension }) {
            ...
            const cooking = JSON.parse(world.getDynamicProperty(blockGeneratedDynamicPropertyId(block)));
            // get state
            const isGrillOpen = block.permutation.getState(`${NAMESPACE}:grill_open`);
            ...
            if (cooking.status == 0 && (!isGrillOpen || !selectedItem || !rawFoodList.has(selectedItem.typeId))) {
                ...
                // set true or false
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:grill_open`, isGrillOpen ? false : true));
                return;
            }
