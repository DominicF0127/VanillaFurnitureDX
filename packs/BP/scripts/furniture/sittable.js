...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:sittable`, {
        onPlayerInteract({ block, dimension, player }) {
            ...
            if (selectedItem && (dyeList.has(selectedItem.typeId) || selectedItem.typeId == "minecraft:wooden_axe"))
                return;
            // get set position
            ...
            // trigger an event
            if (seatPosition)
                sittable.triggerEvent(`${seatPosition}`);
            // add player as the rider of sittable entity
            sittable.getComponent(EntityComponentTypes.Rideable)?.addRider(player);
            // playsound
