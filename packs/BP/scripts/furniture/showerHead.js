...
system.beforeEvents.startup.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:shower_head`, {
        onPlayerInteract({ block }) {
            // state
            const isShowerOn = block.permutation.getState(`${NAMESPACE}:shower_on`);
            // update state
            block.setPermutation(block.permutation.withState(`${NAMESPACE}:shower_on`, isShowerOn ? false : true));
            // playsound
