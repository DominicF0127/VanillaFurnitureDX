...
system.beforeEvents.startup.subscribe(({blockComponentRegistry}) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:drinking_glass_stack`, {
        onPlayerInteract({ block, player, dimension }) {
            ...
            const drinkingGlassStack = <number>block.permutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:drinking_glass_stack`);

            if (selectedItem && selectedItem.typeId == `${NAMESPACE}:drinking_glass` && drinkingGlassStack < 3) {
                // update block state
                block.setPermutation(block.permutation.withState(<keyof BlockStateSuperset>`${NAMESPACE}:drinking_glass_stack`, drinkingGlassStack + 1));
            ...
            if (selectedItem && drinkingGlassStack == 1) {
                ...
                if (!(selectedItem.typeId == `${NAMESPACE}:water_pitcher` || selectedItem.typeId == `${NAMESPACE}:juice`)) return;
        ...
        onPlayerBreak({ block, brokenBlockPermutation, dimension }) {
            const drinkingGlassStack = <number>brokenBlockPermutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:drinking_glass_stack`);
            // spawn loot
...
world.afterEvents.blockExplode.subscribe(({ block, explodedBlockPermutation, dimension }) => {
    if (explodedBlockPermutation.type.id == `${NAMESPACE}:drinking_glass`) {
        const drinkingGlassStack = <number>explodedBlockPermutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:drinking_glass_stack`);
        // spawn loot
