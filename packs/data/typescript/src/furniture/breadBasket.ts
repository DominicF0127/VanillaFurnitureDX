...
system.beforeEvents.startup.subscribe(({blockComponentRegistry}) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:bread_basket`, {
        onPlayerInteract({ block, player, dimension }) {
            ...
            const breadStack = <number>block.permutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:bread_stack`);

            if (selectedItem && selectedItem.typeId == "minecraft:bread" && breadStack < 5) {
                // update block state
                block.setPermutation(block.permutation.withState(<keyof BlockStateSuperset>`${NAMESPACE}:bread_stack`, breadStack + 1));
        ...
        onPlayerBreak({ block, brokenBlockPermutation, dimension }) {
            const breadStack = <number>brokenBlockPermutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:bread_stack`);
            // spawn loot
            if (breadStack > 0)
...
world.afterEvents.blockExplode.subscribe(({ block, explodedBlockPermutation, dimension }) => {
    if (explodedBlockPermutation.type.id == `${NAMESPACE}:bread_basket`) {
        const breadStack = <number>explodedBlockPermutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:bread_stack`);
        // spawn loot
        if (breadStack > 0)
            dimension.spawnItem(new ItemStack("minecraft:bread", breadStack), block.center());
