    ...
    beforeEvents.blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:garden_lanterns`, {
        onPlace({block}) {
            if (block.typeId == `${NAMESPACE}:garden_lantern` && block.above().typeId.includes("_chain")) {
                block.setPermutation(block.permutation.withState(`${NAMESPACE}:hanging` as keyof BlockStateSuperset, true));
            }
        },
        onTick({block, dimension}) {
            ...
            if (block.typeId == `${NAMESPACE}:garden_lantern`) {
                // get block state
                const isHanging = block.permutation.getState(`${NAMESPACE}:hanging` as keyof BlockStateSuperset) as boolean;
                // set loc
