...
system.beforeEvents.startup.subscribe(({blockComponentRegistry}) => {
    blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:record_player`, {
        onPlayerInteract(event) {
            ...
            const playRecordState = <number>block.permutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:play_record`);
            const selectedItem = getPlayerSelectedItem(player);
            if (playRecordState == 0 && (selectedItem == undefined || !musicDiscList.includes(selectedItem.typeId))) return;
            ...
            if (playRecordState == 0 && selectedItem !== undefined && musicDiscList.includes(selectedItem.typeId)) {
                ...
                // update block state
                block.setPermutation(block.permutation.withState(<keyof BlockStateSuperset>`${NAMESPACE}:play_record`, musicDiscIndex));
                // decrement stack
                ...
                // update block state
                block.setPermutation(block.permutation.withState(<keyof BlockStateSuperset>`${NAMESPACE}:play_record`, 0));
        ...
        onPlayerBreak({block, brokenBlockPermutation, dimension}) {
            const playRecordState = <number>brokenBlockPermutation.getState(<keyof BlockStateSuperset>`${NAMESPACE}:play_record`);
            if (playRecordState ==0) return;
            const musicDisc = musicDiscList[playRecordState - 1];
            // return the music disc
            dimension.spawnItem(new ItemStack(musicDisc), block.center());
            // stop sound
