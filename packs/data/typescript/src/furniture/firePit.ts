...
system.beforeEvents.startup.subscribe((beforeEvents) => {
    beforeEvents.blockComponentRegistry.registerCustomComponent(`${NAMESPACE}:fire_pit`, {
        onTick({block, dimension}) {
            const isLit = block.permutation.getState(`${NAMESPACE}:lit` as keyof BlockStateSuperset) as boolean;
            if (isLit == true) {
        ...
        onPlayerInteract({block, player, dimension}) {
            ...
            if (!playerSelectedItem || playerSelectedItem.typeId !== "minecraft:flint_and_steel") return;

            const isLit = block.permutation.getState(`${NAMESPACE}:lit` as keyof BlockStateSuperset) as boolean;
            block.setPermutation(
                block.permutation.withState(`${NAMESPACE}:lit` as keyof BlockStateSuperset, isLit ? false : true)
            );
